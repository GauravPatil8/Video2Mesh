...
def knn_pcl(pcl0, pcl1, feat, K):
    """K-nearest-neighbour lookup (scipy KDTree, no pytorch3d)."""
    src = pcl0[..., :3].detach().cpu().numpy()
    tgt = pcl1[..., :3].detach().cpu().numpy()
    tree = KDTree(tgt)
    # K+1 because the original code skipped the first (self) neighbour
    _, nn_idx_np = tree.query(src, k=K + 1)
    nn_idx_np = nn_idx_np[:, 1:]  # drop self
    nn_idx_t = torch.from_numpy(nn_idx_np).to(pcl1.device)
    nn_vtx = pcl1[nn_idx_t]  # (N, K, 3)
    nn_vtx = torch.mean(nn_vtx, dim=1)
    nn_feat = torch.mean(feat[nn_idx_t], dim=1)
